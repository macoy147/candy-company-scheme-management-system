...
//? Prints text to the console one character at a time, simulating a typewriter effect.
...
//? Displays a loading screen with a progress bar and animated dots.
void loadingScreen()
{
...
//? Creates and initializes a new node in the company tree.
...
//? Writes the details of a node to a file
...
//? Recursively saves the entire tree structure to a file.
...
//? Searches for a node by name in the tree.
...
//? Loads the tree structure from the textfile database
...
//? Removes an employee and their subordinates from the tree.
...
//? Updates the gold of a node based on the number of candies.
void updateGoldFromCandy(Node *node)
{
    node->gold += 20;
}

//? Distributes candy and updates gold for a node and its ancestors.
...
//? Distributes candy and updates the tree structure.
...
//? Displays the tree structure in a typewriter effect.
...
//? Searches for a node by name and displays its details.
...
int nodeExists(Node *root, const char *name)
{
    if (root == NULL)
        return 0;
    ...
    return nodeExists(root->left, name) || nodeExists(root->right, name);
}
        ...
        switch (menuChoice)
        {
            ...
            case 4:
                ...
                else
                {
                    ...
                    if (!nodeExists(root, removeName))
                    {
                        printf("\nEmployee '%s' does not exist in the company!\n", removeName);
                    }
                    // Don't allow removing the CEO if they have employees
                    else if (strcasecmp(root->name, removeName) == 0 && (root->left != NULL || root->right != NULL))
                    {
